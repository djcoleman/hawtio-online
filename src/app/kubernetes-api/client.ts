...
import {
  toKindName, getName, getNamespace, equals, getErrorObject, masterApiUrl, wsUrl, filterByLabel, prefixForKind,
  namespaced, toCollectionName, fullName, getKind, getApiVersion
} from './helpers';
...
export let k8SClientFactory: K8SClientFactory = new K8SClientFactoryImpl();
...
export function del(options: any) {
  ...
  if (options.object.kind === toKindName(WatchTypes.LIST)) {
    handleListAction(options, (object, successFn, errorFn) => {
      del({
        object: object,
        success: successFn,
        error: errorFn
